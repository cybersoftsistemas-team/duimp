inherited damDuimp: TdamDuimp
    ...
    SQLScripts = <
      ...
      item
        ...
        SQL.Strings = (
          ...
          ',Diferenca = '
          ' CASE '
          '        WHEN ISNULL(TRI.NotaFiscalValor, 0) = 0 '
          '                     THEN 0'
          '        ELSE DVB.Majorado - ISNULL(TRI.NotaFiscalValor, 0)'
          ' END'
          'FROM duimp.tributos_calculados AS DTC'
          'JOIN duimp.tributos_calculados_valores_brl AS DVB'
          #9'ON DTC.Id = DVB.TributoCalculadoId'
  ...
  object qryDPGSel: TFDQuery
    ...
    SQL.Strings = (
      ...
      '('
      '     CodigoReceita varchar(8)'
      '    ,CodigoCF varchar(15)'
      '    ,DescricaoCF varchar(60)'
      '    ,TipoCF char(1)'
      '    ,CustoCF bit'
      '    ,Custo_SeletivoCF bit'
      '    ,Custo_EntradaCF bit'
      '    ,Custo_OutrosCF bit'
      '    ,NivelCF smallint'
      '    ,Modalidade smallint'
      ...
      '('
      '    SELECT TOP (1) Compartilhar_Classificacao'
      '    FROM Configuracao'
      ');'
      ''
      'IF @UseCybersoft = 1'
      ...
      '        ,CF.Tipo'
      '        ,ISNULL(CF.Custo, 0)'
      '        ,ISNULL(CF.Custo_Seletivo, 0)'
      '        ,ISNULL(CF.Custo_Entrada, 0)'
      '        ,ISNULL(CF.Custo_Outros, 0)'
      '        ,CF.Nivel'
      '        ,CF.Modalidade_Importacao'
      '    FROM Cybersoft_Cadastros.dbo.ClassificacaoFinanceira CF'
      '    WHERE CF.Codigo_RFB IS NOT NULL'
      '      AND CF.Desativada = 0;'
      'END'
      'ELSE'
      'BEGIN'
      ...
      '        ,CF.Nivel'
      '        ,CF.Modalidade_Importacao'
      '    FROM ClassificacaoFinanceira CF'
      '    WHERE CF.Codigo_RFB IS NOT NULL'
      '      AND CF.Desativada = 0;'
      'END;'
      ''
      'SELECT'
      '     DUV.DuimpId'
      '    ,ModalidadeCodigo = TIP.Codigo'
      '    ,ModalidadeDescricao = TIP.Descricao'
      'INTO #Modalidades'
      'FROM duimp.capas_itens DCI'
      'JOIN duimp.modalidades MDL'
      '    ON DCI.CaracterizacaoImportacaoIndicador = MDL.Id'
      'JOIN duimp.versoes DUV'
      '    ON DUV.Id = DCI.VersaoId'
      'JOIN TiposProcesso TIP'
      '    ON TIP.Codigo = MDL.TipoProcessoCodigo'
      ...
      '    ,DPG.DuimpId'
      'INTO #Pagamentos'
      'FROM duimp.pagamentos DPG'
      'JOIN'
      '('
      '    SELECT'
      '         DuimpId'
      '        ,VersaoOrigem = MAX(VersaoOrigem)'
      '    FROM duimp.pagamentos'
      '    WHERE DuimpId = @DuimpId'
      '    GROUP BY DuimpId'
      ') M'
      '    ON M.DuimpId = DPG.DuimpId'
      '   AND M.VersaoOrigem = DPG.VersaoOrigem'
      'WHERE DPG.DuimpId = @DuimpId;'
      ''
      'IF EXISTS'
      '('
      '    SELECT 1'
      '    FROM #Modalidades MLD'
      '    FULL OUTER JOIN #Pagamentos PAG'
      '        ON MLD.DuimpId = PAG.DuimpId'
      '    JOIN @Classification CF'
      '        ON CF.CodigoReceita = PAG.CodigoReceita'
      '       AND CF.Modalidade = MLD.ModalidadeCodigo'
      ')'
      'BEGIN'
      '    SELECT'
      '         PAG.Id'
      ...
      '    FULL OUTER JOIN #Pagamentos PAG'
      '        ON MLD.DuimpId = PAG.DuimpId'
      '    LEFT OUTER JOIN @Classification CF'
      '        ON CF.CodigoReceita = PAG.CodigoReceita'
      '       AND CF.Modalidade = MLD.ModalidadeCodigo;'
      'END'
      'ELSE'
      ...
      '    JOIN @Classification CF'
      '        ON CF.CodigoReceita = PAG.CodigoReceita'
      '       AND (CF.Modalidade IS NULL OR CF.Modalidade = 0);'
      'END;'
      ''
